...
class DQNReplayBuffer(object):
    def __init__(self, size, frame_history_len, integer_observations=True):
        ...
        integer_observations: bool (default=True)
            Whether observations are integers or floats (for memory optimisation)
        """
    ...
    def store_frame(self, frame):
        ...
        """
        if self.obs is None:
            self.obs = np.empty([self.size] + list(frame.shape), dtype=np.uint8 if self.integer_observations else np.float32)
